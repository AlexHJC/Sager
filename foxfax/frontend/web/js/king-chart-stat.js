$(document).ready(function(){
	...
	//*******************************************
	/*	THIS IS CRILE STATUS CHART
	/********************************************/
	if( $('#visit-chart, #demo-donut-chart').length > 0 ) {

		$res=$("#netcatcode").attr("netcat").split("##"); 
		 res=$.post($base_url + "dashboard/most_active_contact_chart/"+$res[0]+"/"+$res[1], function(data) {
			demo_donut_chart(data);
	 },"json");
function demo_donut_chart(data)
{	 
$.plot('#visit-chart, #demo-donut-chart', data, {
			...
			colors: ["#d9d9d9", "#5399D6", "#d7ea2b","#5399AC", "#d7ea3X"],
		});
}
	}
